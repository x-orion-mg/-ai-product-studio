...
final class ApiKeyRepository {
	...
	public function all(): array {
		...
		$table = self::tableName();
		// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- $table is a trusted internal table name built from $wpdb->prefix.
		$rows = $wpdb->get_results( "SELECT * FROM {$table} ORDER BY provider ASC, priority ASC", ARRAY_A ) ?: array();

		return array_map( array( ApiKey::class, 'fromRow' ), $rows );
	}
	...
	public function resetErrors( int $id ): void {
		...
		$wpdb->update( self::tableName(), array( 'error_count' => 0 ), array( 'id' => $id ), array( '%d' ), array( '%d' ) );
	}
}
