...
abstract class AbstractController {
	...
	protected function guard(): void {
		if ( ! check_ajax_referer( self::NONCE, 'nonce', false ) ) {
			wp_send_json_error(
				array( 'message' => __( 'Jeton de sécurité invalide. Rechargez la page.', 'ai-product-studio' ) ),
				403
			);
		}
		...
		if ( ! current_user_can( self::CAPABILITY ) ) {
			wp_send_json_error(
				array( 'message' => __( 'Permissions insuffisantes.', 'ai-product-studio' ) ),
				403
			);
		}
	}
	...
	protected function fail( string $message, int $status = 400, array $extra = array() ): void {
